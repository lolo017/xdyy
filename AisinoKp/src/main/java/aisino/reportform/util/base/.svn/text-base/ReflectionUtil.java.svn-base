...
public class ReflectionUtil {
	...
	/**因为数据里的encondition取出是小写，类中的属性名可能有大写字母
	 * 所以通过反射，获取实际类中的属性名
	 * 
	 * @param object
	 * @param encondition
	 */
	public static String getPropertyName(Object object,String encondition){
		Class<? extends Object> clz = object.getClass();
		Field[] fields = clz.getDeclaredFields();
		for(Field f : fields){
			if(f.getName().toLowerCase().equals(encondition.toLowerCase())){
				return f.getName();
			}
		}
		return "";
