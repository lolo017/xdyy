...
<script type="text/javascript">
	...
	var mainRole = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '设置主舱',
			url : sy.contextPath + '/securityJsp/base/MainRoleForm.jsp?id=' + id,
			buttons : [ {
				text : '修改',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
...
// 					<%}%>
					...
					<%}%>	
					<%if (securityUtil.havePermission("/base/syuser!resetPwd")) {%>
					str += sy.formatString('<img class="iconImg ext-icon-cog" title="重置密码" onclick="resetPwd(\'{0}\');"/>', row.id);
					<%}%>
					<%if (securityUtil.havePermission("/base/syuser!leaveUser")) {%>
					str += sy.formatString('<img class="iconImg ext-icon-information" title="离职处理" onclick="leaveUser(\'{0}\',\'{1}\');"/>', row.id,row.name);
					<%}%>
					str += sy.formatString('<img class="iconImg ext-icon-monitor" title="设置主舱" onclick="mainRole(\'{0}\');"/>', row.id);
	...
	function resetPwd(id){
		parent.$.messager.confirm('询问', '您确定要重置该用户密码？', function(r) {
			if (r) {
				$.post(sy.contextPath + '/base/syuser!resetPwd.sy', {
					id : id
				}, function(result) {
					if(result.success){
	...
	function leaveUser(id,name){
		parent.$.messager.confirm('询问', '您确定要将 <b style="font-size:17px">'+name+'</b> 离职处理吗？', function(r) {
			if (r) {
				$.post(sy.contextPath + '/base/syuser!leaveUser.sy', {
					id : id
				}, function(result) {
					if(result.success){
						parent.$.messager.alert('提示', result.msg, 'info');
			...
			<tr>
				<td>
					<form id="searchForm">
						<table>
							<tr>
							<td><input name="QUERY_t#isLeave_S_EQ" type="hidden"  value="0"/></td>
								<td>登录名</td>
							...
							<tr>
								<td colspan="10">
								<a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-user_add',plain:true" onclick="addFun()">用户添加</a>
								</td>
							</tr>
						</table>
					</form>
