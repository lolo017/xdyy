...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.sy.modalDialog({
			title : '添加机构信息',
			url : sy.contextPath + '/securityJsp/base/HRorganizationForm.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var showFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '查看机构信息',
			url : sy.contextPath + '/securityJsp/base/HRorganizationForm.jsp?id=' + id
		});
	};
	...
	var removeFun = function(id) {
		parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
				$.post(sy.contextPath + '/base/HRorganization!delete.sy', {
					id : id
				}, function() {
	...
	var grantFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '机构授权',
			url : sy.contextPath + '/securityJsp/base/HRorganizationGrant.jsp?id=' + id,
			buttons : [ {
				text : '授权',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	...
	$(function() {
		grid = $('#grid').treegrid({
			title : '',
			url : sy.contextPath + '/base/HRorganization!treeGrid.sy',
			idField : 'id',
			treeField : 'name',
			parentField : 'pid',
			rownumbers : true,
...
<body class="easyui-layout" data-options="fit:true,border:false">
	<div id="toolbar" style="display: none;">
		<table>
			<tr>
				<%if (securityUtil.havePermission("/base/HRorganization!save")) {%>
