...
<script type="text/javascript">
	...
	function empChange(){
		var dialog = parent.sy.modalDialog({
			title : '选择员工',
			url : sy.contextPath + '/securityJsp/base/EmpTree.jsp?method=empTreeGrid',
			height:540,
			buttons : [ {
				text : '确认',
				handler : function() {
				...
				if(array.length>0){
				$("#empName").val(array[1]);
				$("input[name='data.empId']").val(array[0]);
				}
				}
			} ]
	...
	<form method="post" class="form">
		<fieldset>
			...
			<table class="table" style="width: 100%;">
				...
				<th>关联员工</th>
				<td>
				<input id="empName"  readonly="readonly"/>&nbsp;<img class="iconImg ext-icon-zoom" title="选择" onclick="empChange();"/>
				<input name="data.empId" type="hidden"/>
				</td>
				</tr>
			...
			<input type="hidden" name="data.isLeave" value="0" />
		</fieldset>
	</form>
</body>
</html>