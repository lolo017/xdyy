...
var deleteProduct = function(id){
	parent.$.messager.confirm('询问', '您确定要删除该产品？', function(r) {
		if (r) {
			$.post(sy.contextPath + '/base/product-list!doNotNeedSecurity_deleteProduct.sy', {
				id : id
			}, function(result) {
				if(result.success){
...
var addProduct = function() {
	var dialog = parent.sy.modalDialog({
		title : '新增产品信息',
		url : sy.contextPath + '/securityJsp/base/AddOrEditProduct.jsp',
		height:250,
		buttons : [ {
			text : '新增产品信息',
			handler : function() {
...
var editProduct = function(id) {
	var dialog = parent.sy.modalDialog({
		title : '产品信息维护',
		url : sy.contextPath + '/securityJsp/base/AddOrEditProduct.jsp?id=' + id,
		height:250,
		buttons : [ {
			text : '信息编辑',
			handler : function() {
...
$(function() {
	grid = $('#grid').datagrid(
					{
						title : '',
						url : sy.contextPath + '/base/product-list!doNotNeedSecurity_getProductList.sy',
						striped : true,
						rownumbers : true,
						pagination : true,
						singleSelect : true,
  ...
  <body class="easyui-layout" data-options="fit:true,border:false">
	...
	<form id="searchForm">
		<table style="font-size:12px;">
			...
			<tr>
				<td colspan="7"><a href="javascript:void(0);" class="easyui-linkbutton" data-options="iconCls:'ext-icon-package_add'" onclick="addProduct()">新增产品信息</a></td>
			</tr>
		</table>
		</form>
	</div>
