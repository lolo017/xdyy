               ...
               },
               {
                 ...
                 },
                 {
                     ...
                     {  title : '操作',
               	      ...
               	      formatter : function(value, row) {
      					var str = '';
    					str += sy.formatString('<img class="iconImg ext-icon-note_edit" title="编辑绩效" onclick="editPer1(\'{0}\');"/>', row.id);
  						str += sy.formatString('<img class="iconImg ext-icon-note_delete" title="删除" onclick="delPer(\'{0}\',\'{1}\');"/>', row.id,row.perDate);
      					return str;
      				}
                       } ]],
...
//	}
	var dialog = parent.sy.modalDialog({
		title : '添加绩效信息',
		url : sy.contextPath + '/securityJsp/base/AddPerformanceFormGr.jsp?empIds='+empId,
		width:1000,
		height:600,
		buttons : [ {
			text : '保存',
  ...
  function  editPer1(id) {
	var dialog = parent.sy.modalDialog({
		title : '编辑绩效信息',
		url : sy.contextPath + '/securityJsp/base/EditPerformanceFormGr.jsp?id=' + id+'&empId='+empId,
		width:1000,
		height:600,
		buttons : [ {
			text : '保存',
  ...
  function delPer(id,date){
	  parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
			if (r) {
				$.post(sy.contextPath + '/base/performance!deletePerformance.sy', {
					id : id,
					empId:empId,
					date:date
				}, function() {
