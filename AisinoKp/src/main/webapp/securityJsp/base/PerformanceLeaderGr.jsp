...
var editBumen=function (dpid,empid){
		var content = '<iframe scrolling="auto" frameborder="0"  src="<%=contextPath%>/securityJsp/base/AchievementsBM.jsp?dpid='+dpid+'&empid='+empid+' "style="border: 0; width: 100%; height: 99%;"></iframe>';     
 var jq = top.jQuery;   
    if (jq("#mainTabs").tabs('exists', "部门指标拟定")){    
        jq("#mainTabs").tabs('select', "部门指标拟定");
        var currTab = jq("#mainTabs").tabs('getTab',"部门指标拟定");  
...
var editFunAchievements=function (empId){
	var content = '<iframe scrolling="auto" frameborder="0"  src="<%=contextPath%>/securityJsp/base/Achievements.jsp?empId='+empId+'"style="border: 0; width: 100%; height: 99%;"></iframe>';     
    var jq = top.jQuery;    
    if (jq("#mainTabs").tabs('exists', "个人指标拟定")){    
        jq("#mainTabs").tabs('select', "个人指标拟定");
        var currTab = jq("#mainTabs").tabs('getTab',"个人指标拟定");  
        jq("#mainTabs").tabs('update', {tab: currTab, options: {content: content, closable: true}});  
    } else {  
...
function performance(empId){
	var content = '<iframe scrolling="auto" frameborder="0"  src="<%=contextPath%>/securityJsp/base/PerformanceGr.jsp?empId='+empId+'"style="border: 0; width: 100%; height: 99%;"></iframe>';     
    var jq = top.jQuery;    
